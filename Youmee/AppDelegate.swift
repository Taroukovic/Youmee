...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        pageContoller.pageIndicatorTintColor = UIColor.lightGray
        pageContoller.currentPageIndicatorTintColor = UIColor(red: 0.000, green: 0.576, blue: 0.918, alpha: 1.00)
        pageContoller.backgroundColor = UIColor.white
        FBSDKApplicationDelegate.sharedInstance().application(application, didFinishLaunchingWithOptions: launchOptions)
      /**  let test = 1
        if (test != 0) // VC initial sera le mapVC
        {
            window = UIWindow(frame: UIScreen.main.bounds)
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            window?.rootViewController = storyboard.instantiateViewController(withIdentifier: "mapVC")
            window?.makeKeyAndVisible()
            print("Déja connecté")
            return true
        } else // VC initial sera le loginVC
        {
            window = UIWindow(frame: UIScreen.main.bounds)
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "Youmee")
